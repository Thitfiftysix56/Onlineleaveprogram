...
function VerifyOtpPage() {
  ...
  useEffect(() => {
    const updateCountdown = () => {
      const remainingMilliseconds =
        (flow.resendAvailableAt || Date.now()) -
        Date.now();
      ...
      <Navigate
        ...
        state={{
          passwordResetMessage:
            'ขั้นตอนรีเซ็ตรหัสผ่านหมดอายุ กรุณาเริ่มใหม่อีกครั้ง',
        }}
      />
    );
  }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setMessage({
        severity: 'success',
        text: 'ส่งรหัสยืนยันอีกครั้งแล้ว',
      });
    } catch (error) {
      const retryAfterSeconds =
        Number(
      ...
      if (
        ...
        setMessage({
          severity: 'error',
          text: `กรุณารอ ${retryAfterSeconds} วินาทีก่อนขอรหัสใหม่`,
        });
      } else {
        setMessage({
          severity: 'error',
          text:
            error.response?.data?.message ||
            'ไม่สามารถส่งรหัสยืนยันอีกครั้งได้',
        });
      }
    } finally {
  ...
  return (
    <PasswordRecoveryLayout
      title="ยืนยันรหัส"
      description="กรอกรหัส 6 หลักที่ส่งไปยัง Email ที่ลงทะเบียนไว้ รหัสจะหมดอายุภายใน 5 นาที"
    >
      {message && (
        <Alert
      ...
      <form onSubmit={handleVerify}>
        ...
        >
          รหัสจะหมดอายุภายใน 5 นาทีหลังจากส่ง
        </Typography>
