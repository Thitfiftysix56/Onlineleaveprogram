...
const genericRequestMessage =
  'ส่งรหัสยืนยันไปยัง Email ที่ลงทะเบียนไว้แล้ว'

const accountNotFoundMessage =
  'ไม่พบบัญชีที่ลงทะเบียนไว้'

const requestWindowMs =
  15 * 60 * 1000

const identifierRequestLimit =
  5

const ipRequestLimit =
  30

const maximumResendsPerWindow =
  3
...
async function minimumResponseDelay(
  startedAt,
) {
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        errorMessage =
          'กรุณาขอรหัสยืนยันใหม่'
      } else if (
        storedOtp.used_at
      ) {
        errorMessage =
          'รหัสยืนยันนี้ถูกใช้งานแล้ว'
      } else if (
        storedOtp.invalidated_at
      ) {
        errorMessage =
          'กรุณาขอรหัสยืนยันใหม่'
      } else if (
        storedOtp.verified_at
      ) {
        errorMessage =
          'รหัสยืนยันนี้ถูกใช้งานแล้ว'
      } else if (
        ...
        errorMessage =
          'กรอกรหัสยืนยันผิดเกินจำนวนครั้งที่กำหนด'
      }
...
export async function resetForgottenPassword(
  ...
  try {
      ...
      await connection.execute(
        `
          SELECT
            ...
            t.invalidated_at,
            o.verified_at,
            o.used_at AS otp_used_at,
            o.invalidated_at AS otp_invalidated_at,
            u.username,
            u.password_hash,
            u.status AS user_status,
            e.email,
          ...
          INNER JOIN password_reset_otps AS o
            ON o.id = t.otp_id
    ...
    console.error(
      'Reset forgotten password error:',
      error,
    )
